...
let curveInd = 0
let imageList = ["ly14.png", "ly15.png","ly16.png","ly-cursive.png",""]
let imageInd = 0
let editMode = 0
let loadInd = 0
...
let makeDot = (x, y) => {
  return `<div class="dot-position" title="{x: ${x}, y: ${y}}" style="left:${x-3}px;top:${y-3}px;"><div class="dot" data-x="${x}" data-y="${y}"></div></div>`
} 
...
$(() => {
	...
	$sqr.on("click", e => {
		if (!editMode) {
			...
			$sqr.append(makeDot(e.offsetX, e.offsetY))
			$result.html(printResults())
			$lines.html("")
			$num.html(`num dots: ${dots.length}`)
  ...
  $curve.on("click", e => {
	...
	$edit.on("click", e => {
		...
		console.log(`Editing ${editMode ? 'on' : 'off'}`)
	})
	
	$stored.on("click", e => {
		// Clear 
		$clear.trigger("click")
		if (loadInd == ly.length) {
			dots = []
		} else {
			// New Dot Values
			dots = ly[loadInd].dots
			// Print Dots
			dots.map(dot => {
				$sqr.append(makeDot(dot.x, dot.y))
			})
			printLines()
			$result.html(printResults())
			$num.html(`num dots: ${dots.length}`)
...
},{"d3":33,"jquery":34}],2:[function(require,module,exports){
